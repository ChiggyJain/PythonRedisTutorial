...
def stream_consumer_read_examples():
    try:

        print(f"Inside func: stream_consumer_read_examples")
        global redisObj
        ...
        msgs = redisObj.xreadgroup(groupName, "Worker-1", {streamName:">"}, count=2)
        print("Messages for worker-1 and now status are pending after reading:", msgs)

        msgs = redisObj.xreadgroup(groupName, "Worker-2", {streamName:">"}, count=2)
        print("Messages for worker-2 and now status are pending after reading:", msgs)

        msgs = redisObj.xreadgroup(groupName, "Worker-3", {streamName:">"}, count=10)
        print("Messages for worker-3 and now status are pending after reading:", msgs)


    except Exception as e:
        print(f"Exception occured inside func stream_consumer_read_examples: {e}")
    

if __name__ == "__main__":
    make_redis_connection()
    stream_consumer_read_examples()
